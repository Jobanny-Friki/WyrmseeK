...
handler.priority_map.update({i[0]: i[2] for i in syslog_extension})  # Enable priorities
...
def _verify_access_impl(path: str, mode_mask: int, target_mode: int, flags: int) -> int:
	fd = os.open(path, flags)
	st = os.fstat(fd)
	if st.st_uid != USER_UID or (st.st_mode & mode_mask):
		os.close(fd)
		msg = f"SECURITY VIOLATION: '{path}' has incorrect ownership or permissions\
 (UID {st.st_uid} mode {oct(st.st_mode)}, expected UID {USER_UID} mode {oct(target_mode)})"
		raise PermissionError(msg)
	return fd


_verify_dir_access = partial(
	_verify_access_impl, mode_mask=0o077, target_mode=0o700, flags=os.O_RDONLY | os.O_DIRECTORY
)

_verify_file_access = partial(
	_verify_access_impl, mode_mask=0o177, target_mode=0o600, flags=os.O_RDONLY | os.O_NOFOLLOW
)
...
def get_command_list(cfg: dict, key: str, default: list[str]) -> list[str]:
	...
	if val is None:
		...
		return []
	result = val if isinstance(val, list) else shlex_split(str(val))
...
class Runner(dbus.service.Object):
	...
	def __init__(self) -> None:
		...
		self.binary = _find_command(get_config_value(self.cfg, "binary", "plocate"), "locate") or ""
		if not self.binary:
			logger.critical("Search binary not found! Install 'plocate' or 'mlocate'")
		else:
		...
		self.opener = get_command_list(self.cfg, "opener", ["mimeo", "handlr", "xdg-open"])

		# NUEVO: Split clipboard commands
		...
		self.process_timeout = get_config_value(self.cfg, "process_timeout", 6.0, float)
		...
		logger.info(
			...
			f"debounce: {self.debounce_ms}ms, timeout: {self.process_timeout}s"
		)
	...
	def save_cache(self) -> None:
		try:
			...
			fd = os.open(CACHE_FILE, os.O_WRONLY | os.O_CREAT | os.O_TRUNC | os.O_NOFOLLOW, 0o600)
			...
			logger.exception(f"Cache save permission error for '{CACHE_FILE}' (needs UID {USER_UID}, mode 0600)")
		except (OSError, ValueError):
	...
	def _run_locate_job(self, norm_query: str, words: tuple[str, ...]) -> None:
		...
		if not self._is_query_stale(norm_query):
			...
			if not final and paths_seen == 0:
				logger.info(f"No results for '{norm_query}' - locate database may need update (run 'updatedb')")

			results = tuple(LightResult(p, *get_icon_and_subtext(p), s) for s, p in final)
			logger.info(
				f"Search completed for '{norm_query}': {paths_seen} paths scanned, {len(results)} results in {elapsed:.2f}s"
			)
	...
	def Match(self, query: str) -> list:
		...
		if norm in self.search_results:
			...
			logger.info(f"Cache hit for query: '{norm}' ({len(self.search_results[norm])} results)")
	...
	def _debounce_callback(self, norm: str, words: tuple) -> bool:
		self._debounce_timer = None
		logger.debug(f"Debounce expired, submitting search job: '{norm}'")
	...
	def _exec_clipboard(self, data: str, mime: str, configured_cmd: list[str]) -> bool:
		...
		Args:
			data: Text to copy
			mime: MIME type (MIME_TEXT or MIME_URI)
			configured_cmd: User-configured clipboard command (can be empty)
